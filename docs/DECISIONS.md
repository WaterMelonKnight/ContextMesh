...
## ADR-020 — Provider availability is bean registration, reported without health probing

**Status:** Accepted. **Decision:** `GET /api/v1/providers` publishes `ModelProviderRegistry`'s contents as `ProviderDescriptor` values (identifier, display name, `BUILT_IN`/`EXTERNAL` kind, optional server-configured default model). A provider is available exactly when its bean is registered; an adapter registers only when it is enabled and validly configured, so an unconfigured provider is absent rather than listed as unusable. The endpoint performs no upstream request and reports configuration readiness, not reachability. Provider credentials stay server-side environment configuration; the browser selects a provider identifier and types a model identifier only. **Why:** `OpenAICompatibleProviderProperties` already fails startup when the adapter is enabled without a valid endpoint and key, so "enabled but unconfigured" is unreachable at runtime and a richer status model would describe states that cannot occur; weakening that startup invariant to produce a `configured=false` value would trade a real safety property for a cosmetic one. Active health checks would turn a status read into an outbound request per page load and would still be stale by the time a turn runs. **Consequences:** the browser cannot explain *why* a specific real provider is missing, so the UI shows a generic configuration hint naming the `CONTEXTMESH_OPENAI_*` variables; an operator misconfiguration surfaces at startup or at the first turn rather than in the status payload. The descriptor must never gain a component that can carry a key, header, or endpoint. Stored, per-user, or per-workspace provider credentials remain out of scope.
