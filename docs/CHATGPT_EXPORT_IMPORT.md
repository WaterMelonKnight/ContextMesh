...
- Parent and child references are checked and parent cycles are rejected. The selected endpoint, explicit current node, and mapping-node count are retained as bounded conversation metadata for provenance and future branch-aware import.
...
- Only `content_type: "text"` with non-empty string `parts` is supported. Parts, Unicode, Markdown, code, whitespace, and newlines are preserved exactly. Images, files, audio, structured tool payloads, and other multimodal content fail validation; no placeholder text is invented.
