...
`source_fingerprint` is lowercase SHA-256 over UTF-8 JSON for the complete normalized imported conversation. The canonical document has fixed field order, preserves message/content-list order, renders enums by their stable names and instants in ISO-8601 form, includes nulls, and recursively sorts metadata object keys while preserving metadata arrays. It contains no persistence IDs or import timestamps. Thus unordered Java `Map` iteration and database-generated values cannot change the fingerprint.

Native conversations have ContextMesh UUID identity, no external identity, no source fingerprint,
and no import timestamp. A database check preserves the inverse imported invariant: imported rows
must retain both fingerprint and import time. Each native message uses its generated message UUID,
rendered as a string in `external_id`, as its provider-neutral stable reference. This is a native
identity, not a fabricated provider ID. Linear appends set `parent_external_id` to the prior stable
message ID. The conversation row is locked while the next sequence is selected and inserted, so
concurrent appends serialize while `(conversation_id, sequence_no)` remains database-unique.
...
| `conversations` | Normalized header: source type/provider, external ID, title, source created/updated, nullable source fingerprint/import time, persistence times, metadata JSONB | PK; workspace-scoped external identity when present; workspace/fingerprint identity otherwise; lifecycle check distinguishes native/imported requirements | Imported source history is immutable; native rows are appendable through Native Talk only. |
| `messages` | Ordered normalized record: conversation, external/native stable ID, zero-based sequence, role, structured content JSONB, source timestamp, parent stable ID, generation provider/model, metadata | PK; unique conversation/sequence and conversation/external ID; workspace/conversation/sequence index and tenant-safe composite FK | Imported messages are immutable; native messages are append-only. Hard-delete with conversation. |
