...
import type { Conversation, ConversationSummary, Origin, Provider } from "./types";
...
export const api = {
  ...
  // Which providers the server has configured. Selectable providers come from here only, so the
  // browser never guesses a provider identifier and never holds provider credentials.
  providers: () => json<Provider[]>("/api/v1/providers"),
